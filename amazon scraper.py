...
async def scraper(url:str)->HTML:
    ...
    async with get_session(service, browser) as session:
        try:
            await asyncio.wait_for(session.get(url), timeout=120)
        except asyncio.TimeoutError:
            return ''

        await asyncio.sleep(20)
        body = await session.get_page_source()
        parsable_html = HTML(html=body)
...
if __name__ == "__main__":    
    ...
    body = asyncio.run(scraper(url.format(product, 1)))

    last_page = int(body.find("ul.a-pagination > li.a-disabled")[-1].text)

    # task_list = []
    # for i in range(1, last_page+1):
    #     task_list.append(
    #         asyncio.run(asyncio.create_task(scraper(url.format(product, i))))
    #     )

    # list_of_bodies = asyncio.run(asyncio.gather(*task_list))
